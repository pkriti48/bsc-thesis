...
occurrences of ```char``` in the word ```word_1 ++ word_2``` generated by appending
```word_1``` and ```word_2``` is the sum of the numbers of occurrences of ```char```
in ```word_1``` and in ```word_2```.
-/
